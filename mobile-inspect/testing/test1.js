(() => {
  ...
  style.textContent = `
    ...
    #devTool textarea {
      ...
      border: none;
      outline: none;
      resize: vertical; /* Changed from 'none' to 'vertical' for better usability */
    }
    #devToolFooter {
      display: flex;
      gap: 4px;
    ...
    #devToolLog { /* Give log more space by default */
        flex-grow: 1;
        max-height: 150px;
    }
    .highlight-inspect {
    ...
    #devToolClose, #devToolMinimize { /* Styles for both close and new minimize button */
    ...
    #devToolMinimize {
        font-size: 20px; /* Larger for '-' or '□' icon */
        line-height: 0.8;
    }
  ...
  const minimizeBtn = box.querySelector("#devToolMinimize"); // Get reference to new minimize button
  ...
  runBtn.onclick = () => {
    ...
    try {
      ...
      const originalError = console.error; // Also capture original error
      const originalWarn = console.warn; // Also capture original warn
  ...
  // --- Close Button Fix ---
  closeBtn.onclick = () => {
    // This line was already correct and should remove the element
    document.body.removeChild(box);
    // Crucially, clear the global flag so it can be re-added
    window.__devToolLoaded = false;
  };

  // --- Collapse Window Style ---
  let isMinimized = false;
  minimizeBtn.onclick = () => {
    isMinimized = !isMinimized;
    if (isMinimized) {
      box.style.height = '40px'; // Set a small height for the header
      cssBox.style.display = 'none'; // Hide textarea
      box.querySelector("#devToolFooter").style.display = 'none'; // Hide footer
      pathBox.style.display = 'none'; // Hide path
      logBox.style.display = 'none'; // Hide log
      minimizeBtn.textContent = '□'; // Change icon to 'square' for maximize
    } else {
      // Restore original height, or responsive height if on mobile
      box.style.height = (window.innerWidth <= 400) ? '60vh' : '280px';
      cssBox.style.display = 'flex'; // Show textarea
      box.querySelector("#devToolFooter").style.display = 'flex'; // Show footer
      pathBox.style.display = 'block'; // Show path
      logBox.style.display = 'block'; // Show log
