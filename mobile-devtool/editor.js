javascript:(function() {
    ...
    if (window.__advancedCssJsEditor) {
        ...
        // If Eruda was loaded by this tool, you might want to destroy it too
    ...
    // --- 1. Inject Eruda for the Console ---
    // You must load Eruda FIRST if you want to integrate its console
    if (!window.eruda) {
        ...
        erudaScript.onload = function() {
            eruda.init({
                tool: ['console', 'elements', 'network', 'resources', 'sources', 'code'], // Tools you want
                defaults: {
                    theme: 'Monokai Pro' // Or 'Dark', 'Light', etc.
                }
            });
            // Optionally hide Eruda initially, and let your UI toggle it
            eruda.hide();
            createEditorUI(); // Only create editor UI after Eruda is ready
        };
    } else {
        // Eruda is already loaded
        eruda.init(); // Re-initialize in case it was destroyed
        eruda.hide(); // Hide if you want your UI to control it
        createEditorUI();
    }

    function createEditorUI() {
        // --- 2. Create Main Editor Container ---
        ...
        editorContainer.innerHTML = `
            ...
            <div class="__editor-tabs">
                ...
                <button class="__tab-btn" data-tab="console">Console</button>
            </div>
            <div class="__editor-content">
                ...
                <div id="__consolePlaceholder" class="__editor-pane"></div> </div>
            <div class="__editor-footer">
                ...
                <button id="__toggleErudaConsoleBtn">Toggle Eruda Console</button>
            </div>
        `;
        document.body.appendChild(editorContainer);

        // --- 3. Inject Styling for Your Editor UI (Eruda-like) ---
        ...
        editorStyle.innerHTML = `
            #__advancedEditorContainer {
                ...
                background-color: #2e3436; /* Eruda's dark background */
                border: 1px solid #4d4d4d; /* Eruda's border color */
                border-radius: 8px;
                box-shadow: 0 4px 10px rgba(0, 0, 0, 0.5);
                z-index: 9999999; /* Higher than Eruda if needed */
                display: flex;
            ...
            .__tab-btn.active {
                color: #007bff; /* Eruda accent blue */
            ...
            #__consolePlaceholder {
                /* This is just a placeholder; Eruda will inject its own UI */
            ...
            .__editor-footer button {
                background-color: #007bff; /* Eruda blue */
                color: white;
                border: none;
                padding: 6px 10px;
                border-radius: 4px;
        ...
        // --- 4. Get Elements and Add Event Listeners ---
        ...
        const toggleErudaConsoleBtn = document.getElementById('__toggleErudaConsoleBtn');
        ...
        applyChangesBtn.addEventListener('click', function() {
            ...
            // APPLY HTML (This is tricky for a live editor.
            // For safety and simplicity, consider these options for a bookmarklet:
            // 1. Replace the innerHTML of a specific target element:
            //    let targetElement = document.getElementById('some-target-id');
            //    if (targetElement) targetElement.innerHTML = htmlEditor.value;
            // 2. Inject new HTML elements:
            //    document.body.insertAdjacentHTML('beforeend', htmlEditor.value);
            // 3. For contentEditable, user directly types. This button might just apply CSS.)
            //
            // For now, let's just make sure contentEditable is toggled
            // if you put live editable HTML in the textarea and expect it to render.
            // Or use the prompt for a specific selector and its new innerHTML.
        ...
        // Toggle Eruda Console visibility
        toggleErudaConsoleBtn.addEventListener('click', function() {
            if (window.eruda) {
                eruda.toggle();
            } else {
                console.warn('Eruda not loaded or initialized.');
            }
        });
        ...
        editorCloseBtn.addEventListener('click', function() {
            ...
            if (window.eruda) eruda.destroy(); // Destroy Eruda if it was loaded by this tool
        ...
        tabButtons.forEach(button => {
            button.addEventListener('click', function() {
                ...
                document.getElementById('__'+ targetTab +'Editor' || '__consolePlaceholder').classList.add('active');

                // Special handling for console tab
                if (targetTab === 'console' && window.eruda) {
                    eruda.show(); // Ensure Eruda is visible when its tab is active
                    // Move Eruda's container if you want it to appear *inside* your editor
                    // This is complex and might not work well due to Eruda's own positioning
                    // eruda.getContainer().style.position = 'static'; // Or similar
                    // document.getElementById('__consolePlaceholder').appendChild(eruda.getContainer());
                } else if (window.eruda) {
                    eruda.hide(); // Hide Eruda when other tabs are active
                }
            });
        });
